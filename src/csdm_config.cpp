	...
	while (!feof(fp))
	{
		...
		if (ptr[0] == '[')
		{
			...
			if (section[0] != '\0')
			{
				for (iter = m_Hooks.begin(); iter != m_Hooks.end(); iter++)
				{
					...
					if (pHook->section.compare(section) == 0) {
						g_amxxapi.ExecuteForward(pHook->forward, (cell)CFG_DONE, "", section);
					}
				}
			}
			...
			for (iter = m_Hooks.begin(); iter != m_Hooks.end(); iter++)
			{
				...
				if (pHook->section.compare(section) == 0) {
					g_amxxapi.ExecuteForward(pHook->forward, (cell)CFG_RELOAD, "", section);
				}
			}
		}
		else if (section[0] != '\0')
		{
			for (iter = m_Hooks.begin(); iter != m_Hooks.end(); iter++)
			{
				...
				if (pHook->section.compare(section) == 0) {
					g_amxxapi.ExecuteForward(pHook->forward, (cell)CFG_READ, ptr, section);
				}
			}
		}
	}
